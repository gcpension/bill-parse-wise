...
export const TVBusinessForm = ({ selectedPlan, onClose }: TVBusinessFormProps) => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    currentProvider: "", subscriberNumber: "", targetProvider: selectedPlan.company, requestedPackage: selectedPlan.planName,
    equipmentReturnMethod: 'courier', powerOfAttorneyExpiry: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const content = [`תאריך: ${new Date().toLocaleDateString('he-IL')}`, "טופס בקשת מעבר ספק טלוויזיה - לקוח עסקי", `חברה: ${selectedPlan.company}`, `תאגיד: ${formData.companyName}`];
      const pdf = await createHebrewPDF("טופס בקשת מעבר ספק טלוויזיה עסקי", content);
      pdf.save(`tv-business-switch-${formData.companyName.replace(/\s+/g, '-')}.pdf`);
